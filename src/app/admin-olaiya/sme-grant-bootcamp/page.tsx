...
import { Loader2, GraduationCap, Download, Trash2, Eye, Search, RefreshCw, BarChart } from 'lucide-react';
...
const AdminSMEGrantPage = () => {
        ...
        try {
            ...
            if (markAsRead) {
                ...
                if (unread.length > 0) {
                    ...
                    unread.forEach(reg => {
                        batch.update(doc(db, 'smeGrantBootcampRegistrations', reg.id), { read: true });
                    });
                    await batch.commit();
                }
            }
    ...
    const downloadCSV = (dataToExport: Registration[]) => {
        ...
        link.setAttribute("download", "sme_grant_registrations.csv");
    ...
    const handleDelete = async () => {
        ...
        const batch = writeBatch(db);
        selected.forEach(id => {
            batch.delete(doc(db, 'smeGrantBootcampRegistrations', id));
        });
        await batch.commit();
    ...
    const handleMarkAsRead = async () => {
        ...
        const batch = writeBatch(db);
        selected.forEach(id => {
            batch.update(doc(db, 'smeGrantBootcampRegistrations', id), { read: true });
        });
        await batch.commit();
    ...
    return (
        <div>
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
                <h1 className="text-3xl font-bold">SME Grant Bootcamp Registrations</h1>
                <div className="flex items-center gap-2 flex-wrap">
                    <Button variant="outline" size="sm" onClick={() => fetchRegistrations(true)} disabled={loading}>
                        <RefreshCw className={cn("mr-2 h-4 w-4", loading && "animate-spin")} /> Refresh
                    </Button>
                    <Button variant="outline" size="sm" asChild>
                        <Link href="/admin-olaiya/sme-grant-analytics">
...
export default AdminSMEGrantPage;
